...
/*   Updated: 2022/11/24 12:38:00 by twang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t	base_conv(va_list list, char c)
{
	...
	else if (c == 'p')
	{
		...
		size += (ft_putnbr_base((size_t)va_arg(list, long long int), HEXA_LOW));
	}
	else if (c == 'd' || c == 'i')
		...
		return (ft_putnbr_base(va_arg(list, unsigned int), HEXA_LOW));
	else if (c == 'X')
		return (ft_putnbr_base(va_arg(list, unsigned int), HEXA_UP));
	else if (c == '%')
